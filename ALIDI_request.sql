...
--Поля в результате: store_id, cma, art_name, средняя цена продажи
...
--Поля в результате: месяц продажи, cma, период со скидкой\без скидки, кол-во единиц товара, сумма продажи
...
WHERE st.city_desc = 'Nizhny Novgorod'
     ...
     AND '2018-03-15' BETWEEN pd.date_from and pd.date_to #запрос отработал, но из-за формата даьы м. словаться на боевом df
...
--Поля в результате: месяц, накопительный итог
